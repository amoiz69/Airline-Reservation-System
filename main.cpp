...
void AddSamplePassengers() {
    ...
    // Adding passengers for flight 111
    route1.nHops = 1;
    ...
    // Adding passengers for flight 222
...
void FindShortestPaths(const string& startCity, const string& endCity) {
    ...
    // Initialize distances for direct flights
    FlightType* flight = cityList[startIdx].nextDeparture;
    while (flight) {
        int endIdx = cityIndex[flight->endCity];
    ...
    if (dist[startIdx][endIdx] == INT_MAX) {
        ...
        // Reconstruct and print all shortest paths
...
// Recursive function to find all paths between two cities
void FindPaths(int start, int end, const vector<vector<int> >& next, vector<int> currentPath, vector<vector<int> >& allPaths) {
    if (start == end) {
...
void DisplayPassengers(int flightNo) {
    ...
    // Traverse the reservation list to find passengers for the flight
    ReservationType* current = reserveHead;
    while (current) {
    ...
    // Display sorted list of passengers
...
void ListReachableCities(const string& cityName) {
    if (cityIndex.find(cityName) == cityIndex.end()) {
        cout << "City not found in the list.\n";
        return;
    }

    int startIdx = cityIndex[cityName];
    vector<bool> visited(cityCount, false);
    queue<int> cityQueue;

    visited[startIdx] = true;
    cityQueue.push(startIdx);

    cout << "\nCities reachable from " << cityName << ":\n";
    cout << left << setw(10) << "City" << endl;
    cout << string(10, '-') << endl;

    while (!cityQueue.empty()) {
        int currentIdx = cityQueue.front();
        cityQueue.pop();

        // Display the current city
        if (currentIdx != startIdx) {
            cout << left << setw(10) << cityList[currentIdx].cityName << endl;
        }

        // Explore neighbors (cities connected via flights)
        FlightType* flight = cityList[currentIdx].nextDeparture;
        while (flight) {
            int neighborIdx = cityIndex[flight->endCity];
            if (!visited[neighborIdx]) {
                visited[neighborIdx] = true;
                cityQueue.push(neighborIdx);
            }
            flight = flight->nextDeparture;
        }
    }
...
int main() {
    ...
    while (true) {
        ...
        cout << "6. Delete a reservation\n"; // New option for deleting a reservation
        ...
        case 5:
            DisplayReservations();
            break;
        case 6: { // New case for deleting a reservation
        ...
        case 8: { // New case for displaying passengers
            int flightNo;
